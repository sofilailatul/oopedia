				...
				<ContentCard className="border-[#9fc4ff]" title={null}>
					<div className="flex items-start justify-between gap-4 space-y-4">
						<div className="space-y-4">
							<h1 className="text-lg font-bold text-gray-900">{material.material_name}</h1>
				...
				<div className="space-y-4">
					{material.contents && material.contents.length > 0 ? (
						material.contents.map((content) => (
							<ContentCard key={content.id} title={content.title || ''} className="border-[#9fc4ff]">
								{content.image_path && (
									<img
						...
						<ContentCard title="Konten" className="border-[#9fc4ff]">
							<p className="text-gray-500 italic">Konten materi belum tersedia.</p>
						</ContentCard>
					)}
				</div>
			</div>
