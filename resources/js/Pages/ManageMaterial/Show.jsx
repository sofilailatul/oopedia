...
import { FaBookOpen, FaLayerGroup, FaRegClock, FaUserAlt } from 'react-icons/fa';
	...
	return (
		...
		>
			<div className="mx-auto space-y-9">
				<div className="relative overflow-hidden rounded-3xl border border-cyan-100 bg-gradient-to-br from-cyan-50 via-blue-50 to-indigo-100 p-6 shadow-[0_18px_50px_rgba(56,189,248,0.18)] sm:p-8">
					<div className="relative space-y-8">
						<div>
							...
							{material.description ? (
								<p className="mt-3 max-w-3xl text-[12px] leading-relaxed text-slate-600 sm:text-[12px]">
									{material.description}
								</p>
							) : null}
						</div>
						...
						<div className="grid gap-3 sm:grid-cols-3">
							<div className="rounded-2xl border border-white/80 bg-white/80 px-4 py-3 shadow-sm backdrop-blur">
								...
								<p className="mt-1 text-[11px] font-bold text-slate-800">{material.author || '-'}</p>
							</div>
							...
							<div className="rounded-2xl border border-white/80 bg-white/80 px-4 py-3 shadow-sm backdrop-blur">
								...
								<p className="mt-1 text-[11px] font-bold text-slate-800">{contentCount} bagian</p>
							</div>
							...
							<div className="rounded-2xl border border-white/80 bg-white/80 px-4 py-3 shadow-sm backdrop-blur">
								...
								<p className="mt-1 text-[11px] font-bold text-slate-800">{displayDate}</p>
							</div>
						</div>
					</div>
				</div>
				...
				<div className="space-y-4">
					{material.contents && material.contents.length > 0 ? (
							...
							>
								...
								<div className="space-y-5 px-8 py-8 sm:px-8 sm:py-8">
									{(content.image_url || content.image_path) && (
										<div className="mx-auto w-full max-w-3xl overflow-hidden rounded-2xl border border-slate-200/70 bg-slate-100/50 shadow-sm">
											<img
												src={content.image_url || `/storage/${content.image_path}`}
						...
						<div className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center shadow-sm">
							<p className="text-[12px] font-medium text-slate-500">Konten materi belum tersedia.</p>
						</div>
					)}
				</div>
			</div>
