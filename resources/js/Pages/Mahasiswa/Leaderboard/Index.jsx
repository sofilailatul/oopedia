  ...
  return (
    ...
    >
      ...
      <div className={`mt-3 rounded-xl px-5 py-2 font-bold text-lg ${isChampion ? "bg-yellow-400/20 text-yellow-700" : "bg-slate-100 text-slate-800"}`}>
        {entry.total_score}
        <span className="block text-[10px] font-normal text-center opacity-60">kuis</span>
      </div>
      <div className="mt-1 flex items-center gap-1 text-[10px] font-bold text-slate-500 bg-slate-50 px-2 py-0.5 rounded-full border border-slate-100">
        <Icons.Star className="w-2.5 h-2.5 text-amber-500" />
        {entry.efficiency_points}
      </div>
    </div>
  );
}
    ...
    <AppLayout title="Leaderboard Kelas" label="Leaderboard">
      <div className="mx-auto px-2 space-y-8">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          {/* Header */}
          <div className="hidden md:grid md:grid-cols-[60px_1fr_100px_120px_100px_60px] px-5 py-3 bg-slate-50 text-xs font-semibold text-slate-500 uppercase tracking-wide border-b border-slate-100">
            <div className="text-center">Rank</div>
            <div >nama mahasiswa</div>
            <div className="text-center">Nilai Kuis</div>
            <div className="text-center">Poin Latihan</div>
            ...
            rankings.map((entry) => {
              ...
              return (
                <div key={entry.user_id}>
                  ...
                  <div
                    className={[
                      "grid grid-cols-[60px_1fr_100px_120px_100px_60px] items-center px-5 py-4 border-b border-slate-50 transition cursor-pointer hover:bg-slate-50/80",
                  ...
                  >
                    ...
                    <div className="text-center">
                      <span className={`text-sm font-bold ${isMe ? "text-blue-600" : "text-slate-900"}`}>
                        {entry.total_score}
                      </span>
                    </div>

                    <div className="text-center">
                      <div className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600 shadow-sm border border-slate-200/50">
