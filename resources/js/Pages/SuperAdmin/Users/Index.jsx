...
export default function Index({ users, filters }) {
	...
	const executeDelete = () => {
		...
		router.delete(route("superadmin.users.destroy", statusModal.userToDelete.id), {
			preserveScroll: true,
			onSuccess: () => {
				setStatusModal({
					isOpen: true,
	...
	return (
		<AppLayout title="Kelola User" label="Kelola User">
					...
					<div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
						...
						<Dropdown>
							...
							<Dropdown.Content width="full">
								<Dropdown.Item onClick={() => handleFilterChange("role", "")} className={!filters?.role ? "bg-sky-50 text-sky-700 font-semibold" : ""}>
									Semua Role
								</Dropdown.Item>
								<Dropdown.Item onClick={() => handleFilterChange("role", "superadmin")} className={filters?.role === "superadmin" ? "bg-sky-50 text-sky-700 font-semibold" : ""}>
									Superadmin
								</Dropdown.Item>
								<Dropdown.Item onClick={() => handleFilterChange("role", "dosen")} className={filters?.role === "dosen" ? "bg-sky-50 text-sky-700 font-semibold" : ""}>
									Dosen
								</Dropdown.Item>
								<Dropdown.Item onClick={() => handleFilterChange("role", "mahasiswa")} className={filters?.role === "mahasiswa" ? "bg-sky-50 text-sky-700 font-semibold" : ""}>
									Mahasiswa
								</Dropdown.Item>
								<Dropdown.Item onClick={() => handleFilterChange("role", "tamu")} className={filters?.role === "tamu" ? "bg-sky-50 text-sky-700 font-semibold" : ""}>
									Tamu
								</Dropdown.Item>
							</Dropdown.Content>
						</Dropdown>
										...
										<tr key={user.id} className="group transition-all hover:bg-slate-50/80">
											...
											<td className="px-6 py-4">
												<span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${
													user.role === 'superadmin' ? 'bg-purple-100 text-purple-700 ring-1 ring-inset ring-purple-600/10' :
													user.role === 'dosen' ? 'bg-sky-100 text-sky-700 ring-1 ring-inset ring-sky-600/10' :
													user.role === 'mahasiswa' ? 'bg-emerald-100 text-emerald-700 ring-1 ring-inset ring-emerald-600/10' :
			...
			{/* Modal Create/Edit */}
			<Modal show={modalState.isOpen && (modalState.type === 'create' || modalState.type === 'edit')} onClose={closeModal} maxWidth="md">
				<form onSubmit={handleSubmit} className="p-6">
					...
					<div className="space-y-4">
						...
						<div>
							<label className="block text-sm font-medium text-slate-700">Password {modalState.type === 'edit' && '(opsional)'}</label>
							<input
								type="password"
								value={data.password}
						...
						<div>
							...
							<Dropdown>
								...
								<Dropdown.Content width="full">
									{['superadmin', 'admin', 'dosen', 'mahasiswa', 'tamu'].map((r) => (
										<Dropdown.Item 
											key={r} 
											onClick={() => setData("role", r)} 
											className={data.role === r ? "bg-sky-50 text-sky-700 font-semibold" : ""}
										>
					...
					<div className="mt-6 flex justify-end gap-3">
						<Button type="button" variant="ghost" color="gray" onClick={closeModal}>
							Batal
						</Button>
			...
			{/* Modal Show */}
			<Modal show={modalState.isOpen && modalState.type === 'show'} onClose={closeModal} maxWidth="sm">
				{modalState.user && (
					<div className="p-6">
						<div className="mb-6 flex items-center justify-between">
							<h2 className="text-lg font-bold text-slate-900">Detail User</h2>
							<Button type="button" variant="ghost" color="gray" size="sm" onClick={closeModal} className="h-8 w-8 !p-0 rounded-full">
						...
						<div className="space-y-6">
							<div className="flex items-center gap-4">
								...
								<div>
									<h3 className="text-lg font-bold text-slate-900">{modalState.user.nama}</h3>
									<p className="text-sm text-slate-500">{modalState.user.email}</p>
								</div>
							</div>
							...
							<div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
								<dl className="space-y-3 text-sm">
									<div className="flex justify-between">
										...
										<dd className="font-medium capitalize text-slate-900">{modalState.user.role}</dd>
									</div>
									<div className="flex justify-between">
										<dt className="text-slate-500">Bergabung</dt>
										<dd className="font-medium text-slate-900">
											{modalState.user.created_at
												? new Date(modalState.user.created_at).toLocaleDateString("id-ID", { day: 'numeric', month: 'long', year: 'numeric' })
												: "-"}
										</dd>
									</div>
									<div className="flex justify-between border-t border-slate-200/60 pt-3">
										...
										<dd className="font-medium text-slate-900 text-right max-w-[60%]">
											{modalState.user.classes?.length > 0 
												? modalState.user.classes.map(c => c.class_name).join(", ") 
												: <span className="text-slate-400 italic">Tidak ada kelas</span>}
										</dd>
									</div>
								</dl>
							</div>
			...
			<StatusModal
				...
				onConfirm={statusModal.type === 'confirm' ? executeDelete : () => setStatusModal(prev => ({ ...prev, isOpen: false }))}
				onCancel={() => setStatusModal(prev => ({ ...prev, isOpen: false }))}
				confirmText={statusModal.type === 'confirm' ? 'Hapus' : 'OK'}
				cancelText="Batal"
			/>
		</AppLayout>
	);
