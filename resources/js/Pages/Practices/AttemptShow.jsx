...
export default function AttemptShow(props) {
  ...
  const { currentDrag, dragHandlers } = useDragDropOrder({
    current,
    answers,
    setDragSelection: actions.setDragSelection,
  });
          ...
          <main className="flex-1">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div className="font-semibold text-sm text-slate-900">
                {currentIndex + 1}. Latihan Soal {questionTypeLabel(cfg?.question_type)}
              </div>
            </div>
                      ...
                      {(current.options ?? []).map((opt) => {
                        ...
                        return (
                          <button
                            key={opt.id}
                            type="button"
                            onClick={() => actions.setMC(current.id, opt.id)}
                    ...
                    <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_260px]">
                        ...
                        <div className="mt-4 space-y-3">
                          {(currentDrag?.slots ?? []).map((slot, idx) => (
                            <div
                              ...
                              onDragStart={(event) =>
                                slot && dragHandlers.handleDragStart(event, { source: "slot", index: idx, text: slot })
                              }
                              onDragOver={(event) => event.preventDefault()}
                              onDrop={(event) => dragHandlers.handleDropOnSlot(event, idx)}
                      ...
                      <div className="rounded-2xl border bg-white p-5">
                        ...
                        <div
                          ...
                          onDragOver={(event) => event.preventDefault()}
                          onDrop={dragHandlers.handleDropOnPool}
                        >
                          {(currentDrag?.pool ?? []).map((item, idx) => (
                            <div
                              ...
                              draggable
                              onDragStart={(event) =>
                                dragHandlers.handleDragStart(event, { source: "pool", index: idx, text: item })
                              }
